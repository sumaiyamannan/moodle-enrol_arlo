 ...
 * @group perform
 */
class mod_perform_webapi_resolver_mutation_manually_delete_participant_instance_testcase extends advanced_testcase {
    ...
    public function test_failed_ajax_query_id_999(): void {
        $participant_instance_id = 999;
        $args['input']['participant_instance_id'] = $participant_instance_id;
        $result = $this->parsed_graphql_operation(self::MUTATION, $args);
        $this->assert_webapi_operation_failed(
            $result,
            "Course or activity not accessible. (You are not logged in)"
